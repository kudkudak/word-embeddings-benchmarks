...
def calculate_purity(y_true, y_pred):
    ...
    for id, cl in enumerate(set(y_true)):
        true_clusters[id] = (y_true == cl).astype("uint")
    for id, cl in enumerate(set(y_pred)):
        pred_clusters[id] = (y_pred == cl).astype("uint")

    M = pred_clusters.dot(true_clusters.T)
    return 1. / len(y_true) * np.sum(np.max(M, axis=1))


def evaluate_categorization(w, X, y, seed=777):
    """
    ...
    logger.info("Purity={:.3f} using affinity={} linkage={}".format(best_purity, 'euclidean', 'ward'))
    for affinity in ["cosine", "euclidean"]:
        for linkage in ["average", "complete"]:
            ...
            logger.info("Purity={:.3f} using affinity={} linkage={}".format(purity, affinity, linkage))
    ...
    logger.info("Purity={:.3f} using KMeans".format(purity))
    best_purity = max(best_purity, purity)
